...
use App\Models\Web\Points;
use App\Models\Web\Customerpointcoupon;
...
class CustomersController extends Controller
{
    ...
    public function points(Request $request)
    {
        ...
        $title = array('pageTitle' => Lang::get("website.Points"));
        $final_theme = $this->theme->theme();
        $result = $this->customer->wishlist($request);
        $finalpoints = points::where('customers_id',$customers_id)->first();
        $customerpointcoupon = Customerpointcoupon::where(['customers_id'=> $customers_id,'status'=> 0])->get();
        $result['finalpoints'] = $finalpoints; 
        $result['customerpointcoupon'] = $customerpointcoupon; 

   
        return view("web.points", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result);
    }
    public function insert(Request $request)
    {
        // dd($request->all());
        $customers_id = auth()->guard('customer')->user()->id;
        $customerpointscheck = Points::where('customers_id',$customers_id)->first();
        ...
        if($customerpointscheck->points > $request->savemoney){
            $amount=intval($request->savemoney);
        }else{
            return response()->json([
                "status" => 2,
                "message" => "You Don't Have point to avail this!"
            ]);
        }
        ...
        $maximum_amount = 9999999999;
            ...
            if (count($couponInfo) > 0) {
                ...
                $coupon_id = $this->Coupon->addcoupon($code, $description,
                    ...
                    $customerpointcoupon =Customerpointcoupon::where('customes_id',$customers_id)->create([
                        'customers_id' => $customers_id,
                        'coupon_id' => $coupon_id,
                        'coupon_code' => $code,
                        'coupon_price' => $request->savemoney,
                    ]);

                    if(null != $customerpointscheck){
                        // update points
                      
                    $pointminus = $customerpointscheck->points - $request->savemoney;
             
                    $customerpoints = Points::where('customers_id',$customers_id)->update(
                        [
                            'customers_id' => $customers_id,
                            'points' => $pointminus,
                        ]
