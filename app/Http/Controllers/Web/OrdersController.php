...
class OrdersController extends Controller
{
    ...
    public function getPaymentMethods()
    {
        ...
        $payments_setting = $this->order->payments_setting_for_paypro();
        
        if ($payments_setting['client_key']->environment == '0') {
            $paypro_enviroment = 'Test';
        } else {
            $paypro_enviroment = 'Live';
        }

        $paypro = array(
            'environment' => $paypro_enviroment,
            'name' => $payments_setting['client_key']->name,
            'public_key' => $payments_setting['client_key']->value,
            'active' => $payments_setting['client_key']->status,
            'payment_currency' => Session::get('currency_code'),
            'payment_method' => $payments_setting['client_key']->payment_method,
        );
